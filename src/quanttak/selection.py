...
class BinaryContinuousAnalyzer:
    ...
    def point_biserial_correlation(self):
        ...
        for column in self.dataframe.columns:
            if self.dataframe[column].dtype != "object" and column != self.target:
                corr, _ = pointbiserialr(self.dataframe[column], self.dataframe[self.target])
    ...
    def mutual_information(self, normalize=False):
        ...
        for column in self.dataframe.columns:
            if self.dataframe[column].dtype != "object" and column != self.target:
                mi = mutual_info_score(self.dataframe[column], self.dataframe[self.target])
                mi_scores[column] = mi / np.log(len(self.dataframe)) if normalize else mi
