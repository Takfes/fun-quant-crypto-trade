...
class HMMRegime:
    ...
    def preprocess_data_vix(self, df):
        ...
        vix["Volatility_VIX"] = vix["Returns_VIX"].rolling(window=self.time_period).std()
    ...
    def preprocess_data_inp(self, df):
        ...
        if self.use_indicators_features:
            ...
            data["ADX"] = talib.ADX(data["High"], data["Low"], data["Close"], timeperiod=self.time_period)
    ...
    def preprocess_hmm_data(self, df):
        ...
        if self.use_vix_features:
            ...
            if combo_vix_nulls != 0:
                warnings.warn("Input Data and VIX Data not aligned... Continuing without VIX Data")
                combo = data.copy()
        else:
            combo = data.copy()
    ...
    def plot_market_regimes_scatter(self, mode="test"):
        if mode == "test":
            ...
            train_end_date = self.fnldata_train.index[-1]  # Get the last date of the training data
        else:
        ...
        ax.xaxis.set_major_formatter(mdates.DateFormatter("%b %Y"))  # Set format for the dates
        ...
        # Customize the plot
        ax.set_title(f"Market Regimes with Close Price {mode.upper()} | {df.shape[0]} rows")
        ax.set_xlabel("Date")
    ...
    def plot_market_regimes_span(self, mode="test"):
        ...
        ax.set_xlabel("Date")
        ax.set_ylabel("Close Price")
        legend_patches = [Patch(color=color, label=regime) for regime, color in regime_colors.items()]
