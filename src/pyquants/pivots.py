...
def find_pivots(close_series, window=5):
    ...
    local_max = close_series == close_series.rolling(window=2 * window + 1, center=True).max()
    local_min = close_series == close_series.rolling(window=2 * window + 1, center=True).min()
