...
def make_target_tpsl(close_prices, tp: float = 0.03, sl: float = -0.03):
    ...
    for i in range(1, len(close_prices)):
        ...
        if pd.isnull(tp_hit_days) and pd.isnull(sl_hit_days):  # * Neither T/P nor S/L is hit
        ...
        elif pd.isnull(sl_hit_days) or (not pd.isnull(tp_hit_days) and tp_hit_days < sl_hit_days):  # * T/P is hit first
            results.at[close_prices.index[i], "hit_date"] = tp_hit_days
            results.at[close_prices.index[i], "patience"] = close_prices.index.get_loc(tp_hit_days) + 1 - i
            results.at[close_prices.index[i], "hit_price"] = close_prices.loc[tp_hit_days]
            results.at[close_prices.index[i], "returns"] = (
                close_prices.loc[tp_hit_days] - close_prices.iloc[i - 1]
            ) / close_prices.iloc[i - 1]  # ! check whether this is i or i-1
            results.at[close_prices.index[i], "hit"] = "TP"

        else:  # * S/L is hit first
            results.at[close_prices.index[i], "hit_date"] = sl_hit_days
            results.at[close_prices.index[i], "patience"] = close_prices.index.get_loc(sl_hit_days) + 1 - i
            results.at[close_prices.index[i], "hit_price"] = close_prices.loc[sl_hit_days]
            results.at[close_prices.index[i], "returns"] = (
                close_prices.loc[sl_hit_days] - close_prices.iloc[i - 1]
            ) / close_prices.iloc[i - 1]  # ! check whether this is i or i-1
            results.at[close_prices.index[i], "hit"] = "SL"

    return results.dropna().assign(hit_binary=lambda x: (x.hit == "TP").astype("int")).join(close_prices, how="inner")
...
def lookahead_downsampling(x_set, y_set, steer, fraction_to_sample, random_seed=None):
    ...
    filtered_df["unique_count"] = filtered_df.groupby("hit_date")["hit_date"].transform("size")
    filtered_df["reciprocal_unique_count"] = 1 / filtered_df["unique_count"]

    # Compute sampling probabilities
    filtered_df["sampling_prob"] = filtered_df["reciprocal_unique_count"] / filtered_df["reciprocal_unique_count"].sum()
