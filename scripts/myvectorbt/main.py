...
def macd_dvg_entry_signal(df):
    ...
    dfc["macd_divergence_signal"] = np.where((dfc["close_lower_low"] == 1) & (dfc["macd_higher_low"] == 1), True, False)
...
# Ensure offline mode for Plotly
pyo.init_notebook_mode(connected=True)

# Setting up Cufflinks
cf.go_offline()

qf = cf.QuantFig(df.head(100), title="First Quant Figure", legend="top", name="GS")
qf.add_bollinger_bands()
qf.add_sma([10, 20], width=2, color=["green", "lightgreen"], legendgroup=True)
qf.add_rsi(periods=20, color="java")
qf.add_bollinger_bands(periods=20, boll_std=2, colors=["magenta", "grey"], fill=True)
qf.add_volume()
# qf.add_volume(secondary_y=True)
qf.add_macd()
...
trademore = trr.merge(cashflow, left_on="Exit Timestamp", right_on="timestamp").drop(columns="timestamp")
...
portfolio.plot(subplots=["cash", "assets", "value", "cum_returns", "trades"])  # .show_svg()
...
assert len([x for x in valid_pairs if x[1] is not None]) == pstats["Total Closed Trades"]
