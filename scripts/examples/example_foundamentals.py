...
from pyquants.data import get_fundamentals_yf, get_symbols
...
def format_data(data):
    ...
    if ("fiftyTwoWeekLow" in df.columns) and ("fiftyTwoWeekHigh" in df.columns):
        fiftyTwoWeekRange = (
            (df["currentPrice"] - df["fiftyTwoWeekLow"]) / (df["fiftyTwoWeekHigh"] - df["fiftyTwoWeekLow"])
        ) * 100
    ...
    numerical_cols = [x for x in df.select_dtypes(include=[float]).columns if x not in percentage_cols]

    # Fix Styling
    def make_pretty(styler):
        style_dict = {}
        style_dict.update(dict.fromkeys(numerical_cols, "{:.2f}"))
        style_dict.update(dict.fromkeys(percentage_cols, "{:.2f}%"))
        style_dict.update(dict.fromkeys(million_cols, "{:,.0f} M"))
...
if __name__ == "__main__":
    ...
    if SAMPLESYMBOLS > 0:
        ...
        OUTPUTPATH = f"data/screener_{ROOTSYMBOL}_{SAMPLESYMBOLS!s}_{TIMETAG}.html".lower()
    ...
    threshold_pct = 0.49
    subsetcols = [x for x in fdata.columns if x not in ["quoteType", "symbol"]]
