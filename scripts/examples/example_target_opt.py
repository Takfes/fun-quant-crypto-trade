...
def scenarios(temp, n=30, lower=-0.6, upper=1.2):
    ...
    for i in range(1, n + 1):
        ...
        temp[first_name] = temp["close"].diff(i).shift(-i).fillna(0) / temp["close"] * 100
        temp[second_name] = np.where((temp[first_name] < lower) | (temp[first_name] > upper), i, n)
        Cumulated_name.append(first_name)
        Cumulated_boundary.append(second_name)
    temp["Period"] = temp[Cumulated_boundary].min(axis=1)
    temp["Outcome"] = temp.apply(lambda x: x["DiffClose" + str(x["Period"]).split(".")[0]], axis=1)
    outcome = temp[
        [
            "open",
            "high",
...
sns.heatmap(pivot_table, cmap="RdYlGn", center=0, annot=True, fmt=".1f", annot_kws={"size": 9})
