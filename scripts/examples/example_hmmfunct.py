...
def prep_hmm_data(
    ...
    if use_indicators_features:
        ...
        data["ADX"] = talib.ADX(data["High"], data["Low"], data["Close"], timeperiod=time_period)
    ...
    if use_vix_features:
        ...
        if combo_vix_nulls != 0:
            warnings.warn("VIX Data and Stock Data are not aligned. Continuing without VIX features")
            combo = data.copy()
    else:
        combo = data.copy()
    ...
    if plot:
        ...
        sns.heatmap(corr, cmap="RdYlGn", center=0, annot=True, fmt=".1f", annot_kws={"size": 9})
