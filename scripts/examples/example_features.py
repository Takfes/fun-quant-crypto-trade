...
def macd_cross_signal(df):
    dfc = df.copy()
    dfc["macd"], dfc["macdsignal"], dfc["macdhist"] = talib.MACD(df.close, fastperiod=12, slowperiod=26, signalperiod=9)
...
def macd_divergence_signal(df):
    ...
    dfc["close_lower_low"] = (dfc["close"] < dfc["close"].rolling(12, closed="left").min()).astype(int)
    dfc["macd_higher_low"] = (dfc["macd"] > dfc["macd"].rolling(12, closed="left").max()).astype(int)
    dfc["macd_divergence_signal"] = np.where((dfc["close_lower_low"] == 1) & (dfc["macd_higher_low"] == 1), 1, 0)
...
def locate_local_maxima(df, sensitivity=10):
    dfc = df.copy()
    dfc["local_maxima"] = (
        (dfc["close"][::-1] > dfc["close"][::-1].rolling(sensitivity, closed="left").max())[::-1]
...
pattern_features = talib.get_function_groups()["Pattern Recognition"]
new_cols = {
    feature.lower(): getattr(talib, feature)(df["open"], df["high"], df["low"], df["close"])
    for feature in pattern_features
}
...
features_drop = df.isnull().sum().sort_values(ascending=False).loc[lambda x: x > x.target].index.tolist()
len(features_drop)
...
SAMPLE_SIZE = 1000
top = df["target"].dropna().sort_values(ascending=False).loc[lambda x: x >= 0.05].sample(SAMPLE_SIZE)

mid = df["target"].dropna().sort_values(ascending=False).loc[lambda x: (x < 0.05) & (x > 0.0)].sample(SAMPLE_SIZE)

bot = df["target"].dropna().sort_values(ascending=False).loc[lambda x: x < 0.0].sample(SAMPLE_SIZE)

[x.shape[0] for x in [top, mid, bot]]
sampled_index = [item for sublist in [x.index for x in [top, mid, bot]] for item in sublist]
len(sampled_index)
