...
ppsdata = ppsdata_out.loc[lambda x: (~x["x"].isin(rawfeatures) & (x["baseline_score"] < x["model_score"]))]

TOP_X_FEATURES = 30
featselect = sorted(list(set(corrma.head(TOP_X_FEATURES)["x"].tolist() + ppsdata.head(TOP_X_FEATURES)["x"].tolist())))
len(featselect)
...
results_aggregated = rsg.loc[lambda x: x["metrics"].isin(metrics)].groupby("metrics").agg(["min", "mean", "std"])
results_aggregated = results_aggregated.loc[metrics, sorted(results_aggregated.columns, key=lambda x: x[1])]
results_aggregated
