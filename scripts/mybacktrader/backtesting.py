...
strategies = {"ma": GoldenCross, "bnh": BuyHold, "dip": BuyDip, "dic": Dictum, "3h": TripleH}
...
def parse_user_input():
    ...
    parser.add_argument(
        "type", help="what kind of asset : ['stock','crypto','crypto15','futures15','futures1']", type=str
    )
...
def get_price_series(type, symbol, con, datasize):
    if type == "stock":
        ...
        price_series = (
            pd.read_sql(sql_string, con).assign(datetime=lambda x: pd.to_datetime(x.datetime)).set_index("datetime")
        )
    elif type == "crypto":
        ...
        price_series = (
            pd.read_sql(sql_string, con).assign(openTime=lambda x: pd.to_datetime(x.openTime)).set_index("openTime")
        )
    elif type == "futures1":
