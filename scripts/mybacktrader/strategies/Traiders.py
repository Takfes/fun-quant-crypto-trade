...
class Traid(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.fast_ema = bt.indicators.EMA(
            self.data.close,
            period=self.params.fastema,
            plotname=f"{self.p.fastema}-EMA",
        )
        self.slow_ema = bt.indicators.EMA(
            self.data.close,
            period=self.params.slowema,
            plotname=f"{self.p.slowema}-EMA",
        )
    ...
    def notify_trade(self, trade):
        ...
        ) if trade.pnl > 0 else self.log("(7) OPERATION LOSS, GROSS %.2f, NET %.2f" % (trade.pnl, trade.pnlcomm))
        self.log(f"{50 * '-'}\n")
