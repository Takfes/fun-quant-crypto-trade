...
def annual_risk_return(data: pd.DataFrame, risk_free_rate: float = 0.017) -> pd.DataFrame:
    ...
    returns = pl_data.select([(pl.col(column).mean().alias(column)) for column in pl_data.columns])
    risk = pl_data.select([(pl.col(column).std().alias(column)) for column in pl_data.columns])
...
def find_below_threshold_missingness(data: pd.DataFrame, threshold: float = 0.0) -> List:
    return (data.isnull().sum() / data.shape[0]).loc[lambda x: x <= threshold].index.tolist()
