...
if PULL_DATA_FROM_DATABASE:
    ...
    formatted_query = histdata_query.format(tickers=tuple(TICKERS), start_date=START_DATE, end_date=END_DATE)
...
assert datac.shape[1] == len(non_missing), "Number of symbols with acceptable missingness does not match"
assert datac.shape[0] - int(datac.shape[0] * THRESHOLD) <= returns.shape[0], (
...
print(f"* Number of symbols with acceptable missingness ({THRESHOLD:.2%}) : {len(non_missing)}")
print(f"* Number of symbols dropped due to missingness {len(TICKERS) - len(non_missing)}")
...
print(f"> Number of days dropped due to missingness {datac.shape[0] - returns.shape[0]}")
...
def objective(weights):
    ...
    return -(portfolio_return / portfolio_stddev)  # Negative Sharpe Ratio (assuming risk-free rate is 0 for simplicity)


# Solve the optimization problem
solution = minimize(objective, weights, method="SLSQP", bounds=bounds, constraints=constraints)
...
opt_port_summary = annual_risk_return(opt_port_ret.to_frame(), risk_free_rate=RISK_FREE_RATE)
...
opt_weight_allocation = dict(zip(returns_to_use_optimization.columns, np.round(optimal_weights, 4)))

opt_weight_allocation_nz = {k: v for k, v in opt_weight_allocation.items() if v != 0}
opt_weight_allocation_nz_sorted = dict(sorted(opt_weight_allocation_nz.items(), key=lambda item: item[1], reverse=True))
...
if SIMULATION_REDUCED_UNIVERSE:
    print(f"Reducing SIMULATION universe to {len(opt_weight_allocation_nz_sorted)} assets")
...
assets_summary = annual_risk_return(returns_to_use_simulation, risk_free_rate=RISK_FREE_RATE)
...
sim_weight_allocation = dict(zip(returns_to_use_simulation, np.round(max_sharpe_weights, 4)))
...
wopt = pd.DataFrame().from_dict(opt_weight_allocation_nz, orient="index", columns=["Optimization Weights"])
wsim = pd.DataFrame().from_dict(sim_weight_allocation, orient="index", columns=["Simulation Weights"])
wdf = wopt.join(wsim, how="outer").sort_values(by="Optimization Weights", ascending=False)
