...
def random_portfolios(num_portfolios, mean_returns, cov_matrix, risk_free_rate):
    ...
    for i in np.arange(num_portfolios):
        ...
        portfolio_std_dev, portfolio_return, _ = portfolio_annualised_performance(weights, mean_returns, cov_matrix)
...
ticker_list = list(set(ticker_major + ticker_promising + ticker_dividend + ticker_remote_work_portfolio))
...
ticker_candidates = (
    (dataclose.isnull().sum() / len(dataclose)).loc[lambda x: x <= ALLOWED_MISSINGNESS_THRESHOLD].index.tolist()
)
...
pre_allocation = {k: v for k, v in zip(ticker_candidates, optimal_weights) if v > MINIMUM_WEIGHT}
...
expected_returns_filtered = {k: expected_returns.get(k) for k in allocation}
covariance_matrix_filtered = returns[list(allocation.keys())].cov()


results, weights = random_portfolios(NUMBER_PORTFOLIO_SIMULATIONS, expected_returns, covariance_matrix, RISK_FREE_RATE)
